  ...
  class InfraManager::Refresher < ManageIQ::Providers::BaseManager::Refresher
    ...
    def collect_inventory_for_targets(ems, targets)
      targets.map do |target|
        ...
        [target, ManageIQ::Providers::Proxmox::Inventory::Collector.new(ems, target)]
      end.to_h
    end

    def parse_targeted_inventory(ems, target, collector)
      _log.info("Parsing inventory for #{target.class} [#{target.name}] id: [#{target.id}]")
      persister = ManageIQ::Providers::Proxmox::Inventory::Persister.new(ems, target)
      parser = ManageIQ::Providers::Proxmox::Inventory::Parser.new(collector, persister)
